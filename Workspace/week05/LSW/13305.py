...
18
"""
cities = int(input("도시 수 : "))
length = list(map(int, input("도로의 길이 : ").split()))
price = list(map(int, input("기름값 : ").split()))

cost = 0
min_cost = price[0]  # 첫 도시의 주유 가격으로 초기화

for i in range(cities - 1):
    # 현재까지의 최소 주유 가격으로 비용 계산
    cost += min_cost * length[i]
    # 다음 도시의 가격이 더 저렴하면 min_cost 갱신
    if price[i + 1] < min_cost:
        min_cost = price[i + 1]
